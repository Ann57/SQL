...
select * from phones_apple 
where price > 718.6666666666666667;

select * from phones_apple 
where price in (400, 700, 800);
...
select ram, avg(price), min(price), max(price), count(*) from phones_samsung 
group by (ram);

select front_camera, avg(price), min(price), max(price), count(*) from phones_samsung 
